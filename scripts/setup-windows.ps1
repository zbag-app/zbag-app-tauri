...
<#
.SYNOPSIS
    Setup script for building zbag Desktop on Windows.
...
Write-Host "=== zbag Windows Build Setup ===" -ForegroundColor Cyan
Write-Host ""
...
if (Test-Path $opensslPath) {
    ...
    if (Test-Path $libcryptoPath) {
        ...
        if (Test-Path (Join-Path $vcLibPath "libcrypto.lib")) {
            ...
            Write-Host "See HOW_TO_BUILD_ZBAG_ON_WINDOWS.md troubleshooting section." -ForegroundColor Yellow
        }
    }
} else {
...
Write-Host "  cd <your-zbag-clone-directory>  # Navigate to where you cloned zbag" -ForegroundColor White
...
Write-Host "  target\release\bundle\nsis\zbag_*_x64-setup.exe" -ForegroundColor White
