...
use zbag_core::domain::{Network, ServerInfo};
...
use zbag_engine::error::{find_engine_ipc_error, ipc_err};
use zbag_engine::grpc_url::validate_grpc_url;
...
pub async fn add_server(
    ...
    let client = zbag_network::grpc_client::GrpcClient::new_with_tor(
        grpc_url.to_string(),
    ...
    zbag_engine::db::server_meta::insert_server(mgr.app_db().conn(), &server, now_ms)
...
pub fn set_default_server(
    ...
    let server = zbag_engine::db::server_meta::get_server(mgr.app_db().conn(), request.server_id)
        .map_err(|e| anyhow::anyhow!(e))?
    ...
    zbag_engine::db::server_meta::set_default_server(
...
pub fn list_servers(state: &AppState) -> anyhow::Result<ListServersResponse> {
    let mgr = state.wallet_manager.lock().expect("mutex poisoned");
    let servers = zbag_engine::db::server_meta::list_servers(mgr.app_db().conn())
        .map_err(|e| anyhow::anyhow!(e))?;
...
pub async fn test_server(
    ...
    let server = {
        let mgr = state.wallet_manager.lock().expect("mutex poisoned");
        zbag_engine::db::server_meta::get_server(mgr.app_db().conn(), request.server_id)
            .map_err(|e| anyhow::anyhow!(e))?
            .ok_or_else(|| ipc_err(errors::INVALID_REQUEST, "server not found"))?
    };
    ...
    match probe {
        Ok(chain_name) => {
            ...
            let now_ms = system_time_to_unix_ms(std::time::SystemTime::now())?;
            let mgr = state.wallet_manager.lock().expect("mutex poisoned");
            let _ = zbag_engine::db::server_meta::update_last_success_at(
                mgr.app_db().conn(),
                server.id,
                now_ms,
            );
