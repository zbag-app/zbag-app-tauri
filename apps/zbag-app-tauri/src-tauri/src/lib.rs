...
pub fn cef_runtime_args() -> Vec<(String, Option<String>)> {
    ...
    #[cfg(target_os = "macos")]
    if std::env::var("ZBAG_USE_SYSTEM_KEYCHAIN").as_deref() != Ok("1") {
...
fn cef_runtime_cache_path(bundle_identifier: &str) -> PathBuf {
    let stamp = std::time::SystemTime::now()
        ...
        .unwrap_or_default();
    std::env::temp_dir().join("zbag-cef").join(format!(
...
fn purge_stale_temp_cef_caches(bundle_identifier: &str, runtime_cache_path: &PathBuf) {
    let temp_cef_root = std::env::temp_dir().join("zbag-cef");
...
pub fn run() {
    ...
    run_with_invoke_handler(tauri::generate_handler![
        // Wallet
        commands::wallet::zbag_create_wallet,
        commands::wallet::zbag_load_wallet,
        commands::wallet::zbag_list_wallets,
        commands::wallet::zbag_get_wallet_status,
        commands::wallet::zbag_unlock_wallet,
        commands::wallet::zbag_lock_wallet,
        commands::wallet::zbag_reauth_wallet,
        commands::wallet::zbag_view_seed_phrase,
        commands::wallet::zbag_logout_wallet,
        // Address
        commands::address::zbag_get_receive_address,
        // Sync
        commands::sync::zbag_start_sync,
        commands::sync::zbag_stop_sync,
        commands::sync::zbag_get_sync_progress,
        // Balance
        commands::balance::zbag_get_balance,
        // Transactions
        commands::transaction::zbag_list_transactions,
        commands::transaction::zbag_prepare_send,
        commands::transaction::zbag_confirm_send,
        commands::transaction::zbag_cancel_send,
        commands::transaction::zbag_retry_broadcast,
        commands::transaction::zbag_shield_funds,
        // Jobs (async operations)
        commands::job::zbag_start_send_job,
        commands::job::zbag_start_shield_job,
        commands::job::zbag_cancel_job,
        commands::job::zbag_get_job_status,
        commands::job::zbag_list_jobs,
        // Backup
        commands::backup::zbag_get_backup_challenge,
        commands::backup::zbag_verify_backup,
        commands::backup::zbag_restore_wallet,
        // Keystone
        commands::keystone::zbag_import_ufvk,
        commands::keystone::zbag_build_signing_request,
        commands::keystone::zbag_finalize_signing,
        commands::keystone::zbag_create_keystone_wallet,
        // Swaps
        commands::swap::zbag_request_swap_quote,
        commands::swap::zbag_start_swap,
        commands::swap::zbag_get_swap_status,
        commands::swap::zbag_list_swaps,
        commands::swap::zbag_get_supported_tokens,
        commands::swap::zbag_refresh_swap_status,
        commands::swap::zbag_resume_pending_swaps,
        // Tor
        commands::tor::zbag_set_tor_enabled,
        commands::tor::zbag_get_tor_state,
        // Logs
        commands::logs::zbag_get_log_location,
        // Servers
        commands::server::zbag_add_server,
        commands::server::zbag_set_default_server,
        commands::server::zbag_test_server,
        commands::server::zbag_list_servers,
        // Version
        commands::version::zbag_get_version,
        // Exchange Rate
        commands::exchange_rate::zbag_get_fiat_settings,
        commands::exchange_rate::zbag_set_fiat_settings,
        commands::exchange_rate::zbag_get_exchange_rate,
    ]);
}
    ...
    F: Fn(tauri::ipc::Invoke<AppRuntime>) -> bool + Send + Sync + 'static,
{
    ...
    let run_result = builder
        ...
        .setup(|app| {
            ...
            #[cfg(feature = "cef-runtime")]
            if std::env::var("ZBAG_HEADLESS_SMOKE").is_ok() {
                let duration_secs = std::env::var("ZBAG_SMOKE_DURATION_SECS")
                    .ok()
                    .and_then(|raw| raw.parse::<u64>().ok())
                    .filter(|duration| *duration > 0)
                    .unwrap_or(15);

                if let Ok(path) = std::env::var("ZBAG_SMOKE_READY_FILE") {
...
pub fn run_test_bridge_only() {
    ...
    println!("Starting zbag in test-bridge mode...");
