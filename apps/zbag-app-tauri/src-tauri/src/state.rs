...
use zbag_engine::key_store_keychain::KeyStoreKeychain;
use zbag_engine::logging::LoggingGuard;
use zbag_engine::reauth::SystemClock;
use zbag_engine::swap_service::SwapService;
use zbag_engine::sync_service::SyncService;
...
pub struct AppState {
    ...
    pub tor_manager: Arc<zbag_tor::TorManager>,
...
fn zbag_data_root() -> anyhow::Result<PathBuf> {
    ...
    {
        match std::env::var("ZBAG_TEST_HOME") {
            Ok(root) => {
                let trimmed = root.trim();
                if trimmed.is_empty() {
                    return Err(anyhow::anyhow!(
                        "ZBAG_TEST_HOME is set but empty or whitespace; test-bridge requires a non-empty path"
                    ));
                }
            ...
            Err(_) => {
                return Err(anyhow::anyhow!(
                    "ZBAG_TEST_HOME must be set to a non-empty path when test-bridge is enabled"
                ));
            }
        }
    }
