//! Native application menu for zbag.
//!
...
///
/// Keep in sync with `apps/zbag-app-tauri/src/constants/menuEvents.ts`.
pub mod events {
...
fn build_app_menu<R: Runtime>(app: &AppHandle<R>) -> tauri::Result<Submenu<R>> {
    ...
    let quit = PredefinedMenuItem::quit(app, Some("Quit zbag"))?;
    let separator = PredefinedMenuItem::separator(app)?;

    #[cfg(target_os = "macos")]
    {
        let hide = PredefinedMenuItem::hide(app, Some("Hide zbag"))?;
