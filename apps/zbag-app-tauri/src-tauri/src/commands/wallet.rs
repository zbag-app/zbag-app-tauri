...
use zbag_core::domain::{AccountInfo, AccountType, SyncPhase, SyncProgress, WalletLockStatus};
...
#[tauri::command(rename = "zbag_create_wallet")]
pub fn zbag_create_wallet(
    ...
    let (grpc_url, tor_manager) = {
        ...
        let grpc_url =
            zbag_engine::server_resolver::resolve_grpc_url(mgr.app_db(), request.network);
    ...
    let birthday_height = match grpc_url {
        Ok(url) => tauri::async_runtime::block_on(async {
            let fetch_future = zbag_engine::wallet_manager::fetch_birthday_height_for_new_wallet(
                &url,
                tor_manager,
            );
...
#[tauri::command(rename = "zbag_list_wallets")]
pub fn zbag_list_wallets(
    state: State<'_, AppState>,
...
#[tauri::command(rename = "zbag_load_wallet")]
pub fn zbag_load_wallet<R: Runtime>(
...
#[tauri::command(rename = "zbag_unlock_wallet")]
pub fn zbag_unlock_wallet(
    state: State<'_, AppState>,
...
#[tauri::command(rename = "zbag_lock_wallet")]
pub fn zbag_lock_wallet(
    state: State<'_, AppState>,
...
#[tauri::command(rename = "zbag_reauth_wallet")]
pub fn zbag_reauth_wallet(
    state: State<'_, AppState>,
...
#[tauri::command(rename = "zbag_view_seed_phrase")]
pub fn zbag_view_seed_phrase(
    state: State<'_, AppState>,
...
#[tauri::command(rename = "zbag_get_wallet_status")]
pub fn zbag_get_wallet_status(
    state: State<'_, AppState>,
...
#[tauri::command(rename = "zbag_logout_wallet")]
pub fn zbag_logout_wallet(
    state: State<'_, AppState>,
...
fn load_accounts_for_wallet(
    mgr: &mut zbag_engine::wallet_manager::WalletManager,
    wallet_id: uuid::Uuid,
) -> anyhow::Result<Vec<AccountInfo>> {
    let wallet_db_accounts = mgr.list_wallet_db_account_ids(wallet_id)?;
    let meta_accounts =
        zbag_engine::db::account_meta::list_accounts(mgr.app_db().conn(), wallet_id)
...
fn build_load_wallet_response(
    mgr: &mut zbag_engine::wallet_manager::WalletManager,
    wallet_id: uuid::Uuid,
    tx_service: &mut zbag_engine::tx_service::TxService<zbag_engine::reauth::SystemClock>,
...
mod tests {
    ...
    fn load_wallet_returns_empty_accounts_when_locked_then_accounts_after_unlock() {
        use zbag_engine::reauth::SystemClock;
        use zbag_engine::tx_service::TxService;
