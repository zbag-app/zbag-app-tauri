...
use zbag_core::ipc::v1::commands::sync::StartSyncRequest;
...
pub fn set_tor_enabled_impl(
    ...
    map_anyhow(|| {
        ...
        let next_state = state
            ...
            .map_err(|e| {
                zbag_engine::error::ipc_err(errors::TOR_CONNECTION_FAILED, e.to_string())
            })?;

        let updated_at_ms = system_time_to_unix_ms(std::time::SystemTime::now()).map_err(|e| {
            zbag_engine::error::ipc_err(errors::INTERNAL_ERROR, format!("time error: {e}"))
        })?;

        {
            let mgr = state.wallet_manager.lock().expect("mutex poisoned");
            zbag_engine::db::tor_meta::upsert_tor_state(
                mgr.app_db().conn(),
                &next_state,
                updated_at_ms,
            )
