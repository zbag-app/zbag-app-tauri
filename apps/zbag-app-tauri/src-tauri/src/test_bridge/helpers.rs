...
pub fn server_probe_timeout() -> Duration {
    let raw = match std::env::var("ZBAG_TEST_BRIDGE_PROBE_TIMEOUT_MS") {
        Ok(value) => value,
    ...
    match raw.trim().parse::<u64>() {
        ...
        _ => {
            error!(
                value = raw.as_str(),
                "invalid ZBAG_TEST_BRIDGE_PROBE_TIMEOUT_MS; using default"
            );
...
pub fn to_ipc_error(err: anyhow::Error) -> zbag_core::ipc::v1::common::IpcError {
    if let Some(engine) = zbag_engine::error::find_engine_ipc_error(&err) {
        return zbag_core::ipc::v1::common::IpcError {
    ...
    zbag_core::ipc::v1::common::IpcError {
