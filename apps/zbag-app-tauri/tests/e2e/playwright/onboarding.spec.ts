...
test.afterEach(async ({ request }) => {
  if (createdWalletId) {
    ...
    try {
      await request.post(`${TEST_BRIDGE_BASE_URL}/invoke/zbag_stop_sync`, {
    ...
    try {
      await request.post(`${TEST_BRIDGE_BASE_URL}/invoke/zbag_logout_wallet`, {
        data: { request: { schema_version: 1, wallet_id: walletIdToCleanup } },
      });
    } catch (e) {
...
test.describe('Test Bridge API', () => {
  ...
  test('can create a new testnet wallet via test bridge', async ({ request }) => {
    ...
    const response = await request.post(`${TEST_BRIDGE_BASE_URL}/invoke/zbag_create_wallet`, {
      data: {
        request: {
          schema_version: 1,
          name: walletName,
