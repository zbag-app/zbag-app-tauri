...
const toZecTokens = Object.freeze(supportedTokens.filter((t) => t.id !== ZEC_ASSET_ID));
 ...
 */
const fromZecTokens = Object.freeze(supportedTokens.filter((t) => t.id !== ZEC_ASSET_ID));

/** Get tokens that can be swapped to ZEC (excludes ZEC itself). */
export function getToZecTokens(): readonly SupportedToken[] {
  return toZecTokens;
}
 ...
 * Currently identical to `getToZecTokens`, but kept separate since supported routes may diverge.
 */
export function getFromZecTokens(): readonly SupportedToken[] {
  return fromZecTokens;
}
