...
async fn list_servers(data_dir: &Path, output: &OutputMode) -> Result<()> {
    ...
    let servers = {
        ...
        zbag_engine::db::server_meta::list_servers(wm.app_db().conn())
...
async fn add_server(data_dir: &Path, name: &str, url: &str, output: &OutputMode) -> Result<()> {
    ...
    {
        let wm = state.wallet_manager.lock().expect("mutex poisoned");
        zbag_engine::db::server_meta::insert_server(wm.app_db().conn(), &server, now_ms)
            .map_err(|e| anyhow::anyhow!(e))?;
    }
...
async fn set_default_server(
    ...
    let server = {
        let wm = state.wallet_manager.lock().expect("mutex poisoned");
        zbag_engine::db::server_meta::get_server(wm.app_db().conn(), server_id)
            .map_err(|e| anyhow::anyhow!(e))?
...

    {
        ...
        zbag_engine::db::server_meta::set_default_server(wm.app_db_mut().conn_mut(), server_id)
            .map_err(|e| anyhow::anyhow!(e))?;
    }
