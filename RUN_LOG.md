...
- 5-rust-transitive-patch-updates: `cargo update --workspace` was a no-op after the Tauri/JS tiers (`0 packages`). Verified anyway: `make check` and `make tauri-build` passed; bundle still outputs `target/release/bundle/macos/zSTASH.app` and `target/release/bundle/dmg/zSTASH_0.2.1_aarch64.dmg`. Last green remains `819c5d0ed57b74caf9e5ab0d2c347ad2e8f05b92`.
