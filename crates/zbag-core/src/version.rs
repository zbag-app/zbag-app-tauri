...
pub const GIT_COMMIT: &str = match option_env!("ZBAG_GIT_COMMIT") {
    Some(commit) => commit,
    None => "",
};
...
pub const GIT_DESCRIBE: &str = match option_env!("ZBAG_GIT_DESCRIBE") {
    Some(desc) => desc,
    None => "",
};

/// Build timestamp in UTC (e.g., "2026-01-22 14:30:00 UTC").
pub const BUILD_TIMESTAMP: &str = match option_env!("ZBAG_BUILD_TIMESTAMP") {
    Some(ts) => ts,
    None => "",
};
...
pub fn is_release() -> bool {
    option_env!("ZBAG_IS_RELEASE") == Some("true")
}
