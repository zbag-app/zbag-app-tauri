...
//! Build script for zbag-core.
//!
//! Captures build metadata as compile-time environment variables:
//! - `ZBAG_GIT_COMMIT`: Short git commit hash
//! - `ZBAG_BUILD_TIMESTAMP`: UTC build timestamp
//! - `ZBAG_IS_RELEASE`: "true" if HEAD is on a version tag, "false" otherwise
...
fn main() {
    ...
    println!("cargo:rustc-env=ZBAG_BUILD_TIMESTAMP={build_timestamp}");
    ...
    println!("cargo:rustc-env=ZBAG_GIT_COMMIT={git_commit}");
    ...
    println!("cargo:rustc-env=ZBAG_GIT_DESCRIBE={git_describe}");
    ...
    println!(
        "cargo:rustc-env=ZBAG_IS_RELEASE={}",
