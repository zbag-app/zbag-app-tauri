...
use zbag_core::ipc::v1::events::SwapChangedEvent;
use zbag_network::near_intents::AppFee;
...
pub struct SwapService {
    app_db_path: PathBuf,
    near: zbag_network::near_intents::NearIntentsClient,
...
fn next_state_from_remote_status(
    ...
    wallet_id: Uuid,
    swap: &SwapInfo,
    remote_status: &zbag_network::near_intents::RemoteStatus,
) -> SwapState {
    let mapped = zbag_network::near_intents::map_remote_status_to_local_state(remote_status);
...
impl SwapService {
    ...
    pub fn new(
        ...
        Self::new_with_near_client_and_tx(
            ...
            wallet_manager,
            tx_service,
            zbag_network::near_intents::NearIntentsClient::new()?,
        )
    }
        ...
        let tokens = match block_on(async { self.near.get_supported_tokens().await }) {
            ...
            Err(err) => {
                ...
                return Err(match err {
                    zbag_network::near_intents::NearIntentsError::TorNotReady => {
    ...
    pub fn request_swap_quote(
        ...
        const AFFILIATE_RECIPIENT: &str = "zbag.near";
        ...
        let req = zbag_network::near_intents::QuoteRequest {
    ...
    fn start_swap_from_zec(
        ...
        let submit_req = zbag_network::near_intents::DepositSubmitRequest {
            tx_hash: txid,
            deposit_address,
        };
    ...
    pub fn refresh_swap_status(
        ...
        let status_res = block_on(async {
            match tokio::time::timeout(
                ...
                self.near
                    .get_status(zbag_network::near_intents::StatusRequest {
                        deposit_address,
            ...
            {
                Ok(res) => res,
                Err(_) => Err(zbag_network::near_intents::NearIntentsError::Transport(
            ...
            crate::tokio_runtime::spawn(async move {
                ...
                loop {
                    ...
                    let status_res = match tokio::time::timeout(
                        Duration::from_secs(NEAR_STATUS_TIMEOUT_SECS),
                        near.get_status(zbag_network::near_intents::StatusRequest {
                            deposit_address,
                            deposit_memo: swap.deposit_memo.clone(),
                        }),
                    )
                    ...
                    match status_res {
                        ...
                        Err(zbag_network::near_intents::NearIntentsError::RateLimited {
                            retry_after,
                        }) => {
                            backoff = retry_after
