...
use zbag_core::ipc::v1::events::{ServerFailoverEvent, TransactionChangedEvent};
...
use zbag_core::permissions::{create_dir_all_secure, write_file_secure};
...
pub struct TxService<C: Clock> {
    ...
    tor_manager: Option<Arc<zbag_tor::TorManager>>,
}
...
impl<C: Clock> TxService<C> {
    ...
    pub fn set_tor_manager(&mut self, tor_manager: Arc<zbag_tor::TorManager>) {
        self.tor_manager = Some(tor_manager);
    }

    pub fn tor_manager(&self) -> Option<Arc<zbag_tor::TorManager>> {
    ...
    pub fn build_signing_request(
        ...
        let (proposal, pczt_full, pczt_for_signer) = {
            ...
            let pczt_full = zbag_keystone::pczt::encode_pczt_full(&pczt_with_proofs);
            let pczt_for_signer = zbag_keystone::pczt::encode_pczt_for_signer(&pczt_with_proofs);
    ...
    pub fn finalize_signing(
        ...
        let pczt_with_proofs = zbag_keystone::pczt::decode_pczt_full(
        ...
        let pczt_with_sigs =
            zbag_keystone::pczt::decode_pczt_base64(signed_payload).map_err(|e| {
        ...
        let pczt =
            zbag_keystone::pczt::combine_pczts(pczt_with_proofs, pczt_with_sigs).map_err(|e| {
...
fn resolve_wallet_account_uuid(
        ...
        .context("failed to list wallet accounts")?
    {
        ...
        // Check key_source first (software wallets, zbag-tagged imports including HardwareSigner)
...
impl<C: Clock> TxService<C> {
    fn send_transaction_bytes(
        ...
        let client = match self.tor_manager.as_ref() {
            Some(tor) => zbag_network::grpc_client::GrpcClient::new_with_tor(
                grpc_url.to_string(),
                Arc::clone(tor),
            ),
            None => zbag_network::grpc_client::GrpcClient::new(grpc_url.to_string()),
        };
...
mod tests {
    ...
    fn set_backup_not_required(app_db: &AppDb, wallet_id: Uuid) {
        let wallet = zbag_core::domain::WalletInfo {
            id: wallet_id,
    ...
    fn test_proposal_record(wallet_id: Uuid, expires_at: SystemTime) -> ProposalRecord {
        ...
        ProposalRecord {
            ...
            summary: TransactionSummary {
                ...
                recipient_kind: zbag_core::domain::RecipientKind::Orchard,
                amount: "0".to_string(),
                fee: "0".to_string(),
                memo_present: false,
    ...
    fn retry_broadcast_propagates_retry_failure_as_error() {
        ...
        crate::db::account_meta::upsert_account(
            app_db.conn(),
            wallet_id,
            &zbag_core::domain::AccountInfo {
                id: 0,
                name: "Main".to_string(),
                account_type: zbag_core::domain::AccountType::Software,
            },
            0,
        )
    ...
    fn fetch_transaction_memos_parses_text_memo() {
        ...
        assert_eq!(memos[0].kind, zbag_core::domain::MemoKind::Text);
    ...
    fn fetch_transaction_memos_parses_empty_memo() {
        ...
        let memos = super::fetch_transaction_memos(&conn, &txid).expect("fetch memos");
        assert_eq!(memos.len(), 1);
        assert_eq!(memos[0].kind, zbag_core::domain::MemoKind::Empty);
    ...
    fn fetch_transaction_memos_parses_binary_memo() {
        ...
        let memos = super::fetch_transaction_memos(&conn, &txid).expect("fetch memos");
        assert_eq!(memos.len(), 1);
        assert_eq!(memos[0].kind, zbag_core::domain::MemoKind::Binary);
