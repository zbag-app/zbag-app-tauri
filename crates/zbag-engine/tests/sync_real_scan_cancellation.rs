...
use zbag_core::domain::{Network, SyncPhase};
use zbag_core::errors;
use zbag_core::ipc::v1::events::SyncProgressEvent;
use zbag_engine::db::wallet_encryption_meta;
use zbag_engine::encryption;
...
#[test]
#[ignore = "manual long-running test; requires ZBAG_GRPC_URL testnet endpoint and network access"]
fn stop_sync_during_real_scan_workload_stops_progress_and_allows_restart() {
    let _endpoint = std::env::var("ZBAG_GRPC_URL")
        .expect("set ZBAG_GRPC_URL to a reachable testnet lightwalletd endpoint");
