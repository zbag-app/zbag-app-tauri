...
use zbag_core::domain::{Network, SwapInfo, SwapState, SwapType};
...
fn insert_swap_directly(
    ...
    zbag_engine::db::swap_meta::insert_swap(conn, wallet_id, swap)
}
...
fn refresh_swap_status_rejects_wrong_wallet() {
    ...
    let near = zbag_network::near_intents::NearIntentsClient::with_base_url("http://localhost:1")
        .expect("near client");
    let swap_service = SwapService::new_with_near_client(app_db_path, Arc::clone(&mgr), near)
        .expect("create swap service");
