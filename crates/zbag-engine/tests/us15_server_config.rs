...
fn set_default_server_is_scoped_to_server_network() {
    ...
    let before =
        zbag_engine::db::server_meta::list_servers(mgr.app_db().conn()).expect("list servers");
    ...
    zbag_engine::db::server_meta::insert_server(mgr.app_db().conn(), &new_testnet, now_ms)
        .expect("insert server");

    zbag_engine::db::server_meta::set_default_server(mgr.app_db_mut().conn_mut(), new_testnet.id)
        .expect("set default");
