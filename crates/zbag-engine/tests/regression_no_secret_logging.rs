...
fn regression_no_secret_logging() {
    ...
    tracing::subscriber::with_default(subscriber, || {
        ...
        tracing::info!(address = %zbag_engine::logging::redact_address(full_address));
        ...
        *reauth_token_seen.lock().expect("mutex poisoned") = reauth_token.clone();
        ...
        tracing::info!(reauth_token = %zbag_engine::logging::Redacted(&reauth_token));
...

        {
            let mut mgr = wallet_manager.lock().expect("mutex poisoned");
            let tx_service =
                zbag_engine::tx_service::TxService::new(zbag_engine::reauth::SystemClock);
