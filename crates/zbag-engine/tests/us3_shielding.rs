...
use zbag_engine::tx_service::{TxEventHandler, TxService};
...
) -> anyhow::Result<zbag_core::ipc::v1::commands::transaction::ShieldFundsResponse> {
...
fn shield_funds_sweeps_transparent_balance_and_deducts_fee() {
    ...
    let events: Arc<Mutex<Vec<zbag_core::ipc::v1::events::TransactionChangedEvent>>> =
...
fn shield_funds_batches_large_input_sets() {
    ...
    let shield_txs: Vec<_> = txs
        .iter()
        .filter(|t| t.tx_type == zbag_core::domain::TransactionType::Shield)
        .collect();
    assert!(
        shield_txs.len() >= 2,
