...
use zbag_core::domain::{AddressType, Network, ShieldAction, SyncPhase, SyncProgress, SyncStatus};
use zbag_engine::db::{
...
fn wallet_status_tracks_backup_and_shielding_needs() {
    ...
    assert_eq!(
        ...
        zbag_core::domain::BackupAction::Required
    );
    assert_eq!(
        status.privacy_posture,
        zbag_core::domain::PrivacyPosture::NeedsAction
    );
    ...
    assert_eq!(
        status.backup_status,
        zbag_core::domain::BackupAction::Complete
    );
    assert_eq!(
        status.privacy_posture,
        zbag_core::domain::PrivacyPosture::Optimal
    );
