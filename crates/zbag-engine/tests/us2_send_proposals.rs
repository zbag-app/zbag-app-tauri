...
fn prepare_confirm_send_task_keeps_proposal_when_wallet_db_preflight_fails() {
    ...
    .expect("create wallet manager");
    let mut tx_service = TxService::new(zbag_engine::reauth::SystemClock);
    ...
    tx_service.import_proposal_for_execution(
        ...
        wallet.id,
        0,
        zbag_core::ipc::v1::commands::transaction::TransactionSummary {
...
fn prepared_retry_task_revalidates_wallet_unlocked_state_before_lock_free_execution() {
    ...
    let (reauth_token, _expires_at) = mgr
        .reauth_wallet(wallet.id, "pw", ReauthPurpose::Spend)
        .expect("reauth wallet");
    let tx_service = TxService::new(zbag_engine::reauth::SystemClock);
    let task = mgr
