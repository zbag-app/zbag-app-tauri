...
use zbag_engine::db::AppDb;
use zbag_engine::server_resolver::resolve_grpc_url;
use zbag_engine::server_resolver::resolve_grpc_url_with_dev_override;

#[cfg(debug_assertions)]
#[test]
fn dev_override_takes_precedence_in_debug_builds() {
    let (db_path, _cleanup) = common::temp_db_path_with_cleanup("zbag_app_db_server_resolver");
...
fn resolve_grpc_url_reads_env_wrapper_in_debug_builds() {
    const CHILD_MARKER_ENV: &str = "__ZBAG_SERVER_RESOLVER_CHILD__";
    const OVERRIDE_URL: &str = "https://example.invalid";

    if std::env::var_os(CHILD_MARKER_ENV).is_some() {
        let (db_path, _cleanup) =
            common::temp_db_path_with_cleanup("zbag_app_db_server_resolver_child");
        let db = AppDb::open(db_path).expect("db open");
    ...
    let status = std::process::Command::new(current_exe)
        ...
        .env("ZBAG_GRPC_URL", OVERRIDE_URL)
        .status()
...
fn resolve_grpc_url_ignores_env_wrapper_in_release_builds() {
    ...
    if std::env::var_os(CHILD_MARKER_ENV).is_some() {
        let (db_path, _cleanup) =
            common::temp_db_path_with_cleanup("zbag_app_db_server_resolver_release_child");
        let db = AppDb::open(db_path).expect("db open");
