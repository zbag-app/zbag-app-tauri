...
pub fn insert_wallet(conn: &Connection, wallet: &WalletInfo, directory_path: &str) -> rusqlite::Result<()> {
    conn.execute(
...
pub fn get_wallet(conn: &Connection, wallet_id: Uuid) -> rusqlite::Result<Option<(WalletInfo, String)>> {
...
pub fn update_last_opened_at(conn: &Connection, wallet_id: Uuid, last_opened_at: i64) -> rusqlite::Result<()> {
    conn.execute(
...
pub fn set_remember_unlock_enabled(conn: &Connection, wallet_id: Uuid, enabled: bool) -> rusqlite::Result<()> {
    conn.execute(
