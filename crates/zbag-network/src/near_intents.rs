...
use zbag_core::domain::swap::SupportedToken;
...
impl NearIntentsClient {
    ...
    pub fn new_with_tor(tor: std::sync::Arc<zbag_tor::TorManager>) -> anyhow::Result<Self> {
        Ok(Self {
            base_url: DEFAULT_BASE_URL.to_string(),
...
pub fn map_remote_status_to_local_state(status: &RemoteStatus) -> zbag_core::domain::SwapState {
    use zbag_core::domain::SwapState;
    match status {
