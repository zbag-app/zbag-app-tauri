...
use zbag_keystone::payload::{decode_zcash_pczt_ur_cbor, encode_zcash_pczt_ur_cbor};
...
fn decode_pczt_base64_rejects_invalid_base64() {
    let err = zbag_keystone::pczt::decode_pczt_base64("not base64")
...
fn decode_pczt_base64_rejects_invalid_pczt_bytes() {
    ...
    let err = zbag_keystone::pczt::decode_pczt_base64(&payload)
...
fn pczt_encode_for_signer_selectively_redacts_proprietary_fields() {
    ...
    let decoded =
        zbag_keystone::pczt::decode_pczt_base64(&raw_payload).expect("decode should succeed");
    assert_eq!(decoded.global().proprietary().len(), 2);

    // encode_pczt_for_signer should selectively redact
    let redacted_payload = zbag_keystone::pczt::encode_pczt_for_signer(&pczt);
