...
- [ ] T087 [US2] Implement proposal-based send flow in crates/zkore-engine/src/tx_service.rs: prepare_send() creates proposal, returns proposal_id, summary, fee; MUST fund from shielded notes only and MUST return `TRANSPARENT_SPEND_BLOCKED` (not `INSUFFICIENT_FUNDS`) when only transparent funds are available
...
- [ ] T087c [US2] Enforce backend watch-only spend blocking in crates/zkore-engine/src/tx_service.rs: in prepare_send(), load account_type from app metadata for the active wallet + account_id and if `HardwareSigner` (or `WatchOnly` if ever encountered) return WATCH_ONLY_CANNOT_SPEND (E2002); also enforce the same check in confirm_send() as defense-in-depth
- [ ] T088 [US2] Implement proposal storage (in-memory with expiration) in crates/zkore-engine/src/tx_service.rs; proposals expire after 5 minutes (`PROPOSAL_EXPIRED`)
- [ ] T089 [US2] Implement confirm_send() in crates/zkore-engine/src/tx_service.rs: require valid re-auth token, then sign and broadcast from proposal_id; enforce shielded-only funding again as defense-in-depth (no transparent inputs; return `TRANSPARENT_SPEND_BLOCKED`)
...
- [ ] T099 [US2] Create apps/zkore-app-tauri/src/pages/Activity.tsx with transaction list displaying txid, type, value, status, memo_present; use the active `account_id` for ListTransactions
