...
- [ ] T074 [P] [US1] Create apps/zkore-app-tauri/src/pages/Receive.tsx with shielded address display, QR code (qrcode.react), one-click copy; use the active `account_id` (from AccountSelector) for GetReceiveAddress
- [ ] T074a [P] [US1] Add transparent “compatibility address” toggle to apps/zkore-app-tauri/src/pages/Receive.tsx: when enabled, fetch/display AddressType::Transparent for the active `account_id` with clear labeling + explanation per FR-018 (receive-only; requires shielding before spending)
...
- [ ] T083 [US1] Create apps/zkore-app-tauri/src/pages/Home.tsx with AccountSelector + balance display (for the active `account_id`), sync progress, and persistent backup reminder driven by GetWalletStatus (undismissable when status.backup_status === 'Required')
- [ ] T083a [P] [US1] Create apps/zkore-app-tauri/src/pages/UnlockWallet.tsx (or modal) prompting for wallet password and invoking UnlockWallet; include a “remember unlock” toggle initialized from LoadWalletResponse.wallet.remember_unlock_enabled and pass it as UnlockWalletRequest.remember_unlock; show on app launch when wallet is locked
