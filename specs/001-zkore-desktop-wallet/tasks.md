...
- [ ] T104 [US3] Add transparent balance display to apps/zkore-app-tauri/src/pages/Home.tsx (active `account_id`) with "Needs Shielding" label and Shield Now button
- [ ] T105 [US3] Implement `TRANSPARENT_SPEND_BLOCKED` in crates/zkore-engine/src/tx_service.rs for any non-shielding spend path that would use transparent inputs; ShieldFunds is the only flow allowed to spend transparent UTXOs
...
- [ ] T106b [US3] Add password prompt (manual re-auth) to apps/zkore-app-tauri/src/components/wallet/ShieldPrompt.tsx: call ReauthWallet then ShieldFunds (active `account_id`) with reauth_token
