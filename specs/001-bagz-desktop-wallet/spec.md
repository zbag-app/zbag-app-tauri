# Feature Specification: bagZ Desktop Wallet

**Feature Branch**: `001-bagz-desktop-wallet`
**Status**: Complete
...
- Q: How should bagZ protect spend-capable secret material at rest on disk (seed/spending keys)? → A: Encrypt with wallet password; optional OS keychain remember
- Q: When should bagZ prompt for the wallet password to unlock a spend-capable wallet? → A: Prompt on app launch (keychain remember optional); require manual password re-auth for every spend (no keychain)
...
- Q: How should bagZ store transaction memos on disk? → A: Encrypt memos at rest using the wallet password
...
A new user downloads bagZ Desktop and creates a wallet. They can immediately receive shielded ZEC without being forced to complete backup first, but the app persistently reminds them to back up until they verify their seed phrase. They can only spend after backup verification.
...
1. **Given** a user launches bagZ for the first time, **When** they select "Create Wallet", **Then** a new wallet is created in under 60 seconds and a shielded receive address is displayed
...
5. **Given** a user has previously created a wallet, **When** they restart bagZ, **Then** the most recently opened wallet is loaded and they can resume without re-creating a wallet
...
A user imports a Unified Full Viewing Key from their Keystone hardware wallet. bagZ displays balances and transactions but cannot spend. Spending requires air-gapped signing via the Keystone device.
...
A user with a Keystone watch-only account initiates a send. bagZ generates an unsigned transaction displayed as a QR code. The user scans this on Keystone, signs, and scans the signed response back into bagZ using webcam or imports via microSD.
