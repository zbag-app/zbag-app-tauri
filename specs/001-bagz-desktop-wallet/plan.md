# Implementation Plan: bagZ Desktop Wallet

**Branch**: `001-bagz-desktop-wallet` | **Spec**: [spec.md](./spec.md)
**Input**: Feature specification from `/specs/001-bagz-desktop-wallet/spec.md`
...
**Storage**: Encrypted wallet DB (zcash_client_sqlite-backed) + separate SQLite app metadata DB
  ...
  - Wallet directory structure with network separation:
    - `~/.bagz/wallets/mainnet/{wallet-id}/` (mainnet wallets)
    - `~/.bagz/wallets/testnet/{wallet-id}/` (testnet wallets)
...
**Logging**: tracing + tracing-appender for structured file logging with daily rotation. Logs stored at `~/.bagz/logs/`. No remote telemetry. Sensitive data (memos, full addresses) redacted by default.
...
│   └── tests/
├── bagz-engine/                  # Wallet engine wrapping librustzcash
│   ├── src/
│   │   ├── lib.rs
...
│   └── tests/
├── bagz-network/                 # gRPC + HTTP clients, transport abstraction
│   ├── src/
│   │   ├── lib.rs
...
│   └── tests/
└── bagz-tor/                     # Tor manager (embedded Arti)
    ├── src/
    │   ├── lib.rs
...
apps/
├── bagz-app-tauri/               # Tauri application shell
│   ├── src-tauri/
│   │   ├── src/
│   │   │   ├── main.rs
...
└── bagz-ui/                      # (Optional) shared UI package if needed
...
- **bagz-core**: Domain types and IPC contracts (Principle IV)
- **bagz-engine**: Wallet operations with secrets (Principle I)
- **bagz-network**: Transport abstraction for Tor fail-closed (Principle III)
- **bagz-keystone**: PCZT/signing with anti-fingerprinting (Principle I)
- **bagz-tor**: Isolated Tor state machine (Principle III)
...
- bagZ uses **SQLCipher** (SQLite page-level encryption) for the wallet DB (transaction history, balances, addresses, note metadata). The encryption mechanism MUST encrypt the entire DB file at rest (not just selected columns).
- bagZ uses a per-wallet **key hierarchy**:
