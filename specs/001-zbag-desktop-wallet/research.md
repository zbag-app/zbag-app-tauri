# Research: zbag Desktop Wallet

**Branch**: `001-zbag-desktop-wallet`
**Status**: Complete
...
- **Swap sequence (zbag integration)**:
...
**Implementation Notes**:
- Command prefix: `zbag_` for all Tauri commands
...
- Development/CI only: `ZBAG_GRPC_URL` may override the default lightwalletd endpoint for developer workflows and CI. Production builds should rely on persisted server configuration (FR-052 through FR-055) and MUST NOT silently override user-selected servers via environment variables.
...
**Implementation Notes**:
- Log location: `~/.zbag/logs/zbag.YYYY-MM-DD.log` (daily rotation, retention policy enforced)
...
**Audit summary**:
- **Tauri config** (`apps/zbag-app-tauri/src-tauri/tauri.conf.json`): No crash reporter configuration; no remote telemetry settings.
- **Tauri plugins** (`apps/zbag-app-tauri/src-tauri/Cargo.toml`): Only `tauri-plugin-opener` (no telemetry/crash plugins).
- **Frontend deps** (`apps/zbag-app-tauri/package.json`): No analytics/crash SDK dependencies (e.g., Sentry, PostHog, Segment, Amplitude).
...
zbag pins and enforces Rust **1.92.0** (edition 2024) via `rust-toolchain.toml` and the workspace `rust-version` to align with librustzcash/Zashi. If this minimum changes, update `rust-toolchain.toml`, the workspace `rust-version`, and CI together.
