# Implementation Plan: zbag Desktop Wallet

**Branch**: `001-zbag-desktop-wallet` | **Spec**: [spec.md](./spec.md)
**Input**: Feature specification from `/specs/001-zbag-desktop-wallet/spec.md`
...
**Storage**: Encrypted wallet DB (zcash_client_sqlite-backed) + separate SQLite app metadata DB
  ...
  - Wallet directory structure with network separation:
    - `~/.zbag/wallets/mainnet/{wallet-id}/` (mainnet wallets)
    - `~/.zbag/wallets/testnet/{wallet-id}/` (testnet wallets)
...
**Logging**: tracing + tracing-appender for structured file logging with daily rotation. Logs stored at `~/.zbag/logs/`. No remote telemetry. Sensitive data (memos, full addresses) redacted by default.
...
│   └── tests/
├── zbag-engine/                  # Wallet engine wrapping librustzcash
│   ├── src/
│   │   ├── lib.rs
...
│   └── tests/
├── zbag-network/                 # gRPC + HTTP clients, transport abstraction
│   ├── src/
│   │   ├── lib.rs
...
│   └── tests/
└── zbag-tor/                     # Tor manager (embedded Arti)
    ├── src/
    │   ├── lib.rs
...
apps/
├── zbag-app-tauri/               # Tauri application shell
│   ├── src-tauri/
│   │   ├── src/
│   │   │   ├── main.rs
...
└── zbag-ui/                      # (Optional) shared UI package if needed
...
- **zbag-core**: Domain types and IPC contracts (Principle IV)
- **zbag-engine**: Wallet operations with secrets (Principle I)
- **zbag-network**: Transport abstraction for Tor fail-closed (Principle III)
- **zbag-keystone**: PCZT/signing with anti-fingerprinting (Principle I)
- **zbag-tor**: Isolated Tor state machine (Principle III)
...
- zbag uses **SQLCipher** (SQLite page-level encryption) for the wallet DB (transaction history, balances, addresses, note metadata). The encryption mechanism MUST encrypt the entire DB file at rest (not just selected columns).
- zbag uses a per-wallet **key hierarchy**:
...
- **Testnet**: `https://testnet.zec.rocks`
  ...
  - Development/CI only: configure default server override via `ZBAG_GRPC_URL` environment variable; production builds should rely on persisted server configuration and MUST NOT silently override user-selected servers via environment variables
