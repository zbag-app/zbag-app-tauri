# Quickstart: zbag Desktop Wallet
...
> **Note**: zbag pins and enforces Rust **1.92.0** for builds (via `rust-toolchain.toml` and the workspace `rust-version`) to align with librustzcash/Zashi. If this minimum changes, update `rust-toolchain.toml`, the workspace `rust-version`, and CI together.
...
- zbag does not support a “system SQLCipher” build path; developers and end users do not install SQLCipher separately.
...
```bash
git clone <repository-url> zbag-app-tauri
cd zbag-app-tauri
...
| `test-engine` | Test zbag-engine only |
| `test-core` | Test zbag-core only |
| `test-network` | Test zbag-network only |
| `test-keystone` | Test zbag-keystone only |
| `test-tor` | Test zbag-tor only |
...
bun test            # Frontend tests (from apps/zbag-app-tauri)
```
...
Edit `apps/zbag-app-tauri/src-tauri/tauri.conf.json`:

```json
{
  "$schema": "https://schema.tauri.app/config/2",
  "productName": "zbag Desktop",
  "identifier": "com.zbag.desktop",
  "version": "0.1.0",
  "build": {
...
ZBAG_GRPC_URL=https://testnet.zec.rocks

# Logging
RUST_LOG=info,zbag=debug

# Log file location (logs written here automatically)
# ~/.zbag/logs/zbag.YYYY-MM-DD.log (rotated daily, 7 days retained)
```
...
- name: Integration tests (lightwalletd matrix)
  run: |
    ...
    ZBAG_GRPC_URL=https://zec.rocks cargo test --workspace
    ZBAG_GRPC_URL=https://testnet.zec.rocks cargo test --workspace
