...
This document is the single entry point for CEF usage in zbag: build path, hardening policy, validation gates, and common troubleshooting.
...
- Known-good output paths:
  - `target/release/bundle/macos/zbag.app`
  - `target/release/bundle/dmg/zbag_0.2.2_aarch64.dmg`
...
```bash
open -na target/release/bundle/macos/zbag.app
```
...
4. Bump `apps/zbag-app-tauri/package.json` `@tauri-apps/cli` + `@tauri-apps/api`.
5. Refresh locks:
   ```bash
   cargo update -p tauri -p tauri-build
   cd apps/zbag-app-tauri && bun install
   ```
6. Rebuild and verify the framework actually landed in the bundle:
   ```bash
   make tauri-build
   ls "target/release/bundle/macos/zbag.app/Contents/Frameworks/"
   ```
   Expect `Chromium Embedded Framework.framework` plus five `zbag-app-tauri Helper*.app` bundles. If `Frameworks/` is missing, see **Troubleshooting: missing CEF framework in bundle** below.
7. Launch via `open -na target/release/bundle/macos/zbag.app` and confirm renderer + GPU helper processes spawn.

### Known breakages when bumping Tauri

- **Both `wry` and `cef` features enabled → `error[E0252]: webview_version` defined multiple times.** `tauri/src/lib.rs` re-exports `webview_version` from both runtimes. Fix: in `apps/zbag-app-tauri/src-tauri/Cargo.toml` keep `tauri = { version = "2", default-features = false, features = ["compression", "common-controls-v6", "dynamic-acl"] }`. Do not add `wry` back.
- **`AppHandle` has no default `Runtime` when `wry` is off.** `#[default_runtime(crate::Wry, wry)]` only supplies the default when the `wry` feature is enabled. Every function that holds a Tauri handle needs an explicit runtime generic, e.g. `pub fn f<R: Runtime>(app: &AppHandle<R>)`. Same rule applies to `WebviewWindow<R>`, `Window<R>`, `Manager<R>`. Currently only `apps/zbag-app-tauri/src-tauri/src/windows.rs` is affected, but any new handle-taking function will have to follow suit.
...
# zbag-app-tauri Helper.app
# zbag-app-tauri Helper (GPU).app
# zbag-app-tauri Helper (Renderer).app
# zbag-app-tauri Helper (Plugin).app
# zbag-app-tauri Helper (Alerts).app
```
...
- `~/Library/Caches/app.zbag.desktop/cef/Default/Preferences`
